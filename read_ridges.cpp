...
double calculate_courbure(Segment s, double sk);
void calculate_frenet(Segment s, double sk, double delta_k);
double calculate_dot(Vector_3 v1, Vector_3 v2);


void draw_vector(Vector_3 v, Vector_3 v0, float red, float green, float blue);

void afficherRepereFrenet(const Vector_3& tangente, const Vector_3& normal, const Vector_3& binormal, const Point_3& position, double length) ;

vector<CrestPoint> read_file();
vector<CrestPoint> read();

vector<Segment> update_segments(Spline courbe);
...
//     Vector_3 derivative_first = s.B + 2 * t * s.C + 3 * t2 * s.D;
    ...
    Vector_3 second_derivative = 1.0 / pow(delta_k, 2) * (2 * s.C + 6 * ((sk - 1) / delta_k) * s.D);
    // glm::mat3x2 matrix(first_derivative.x(), second_derivative.x(),
    //                first_derivative.y(), second_derivative.y(),
    //                first_derivative.z(), second_derivative.z());

    // // Calcul du déterminant

    // float determinant = glm::determinant(matrix);

    Vector_3 det = CGAL::cross_product(first_derivative, second_derivative);
    
    double det_norm = calculate_norme(det); 
    double norm_first_derivative = calculate_norme(first_derivative);
    double curvature = det_norm / (norm_first_derivative * norm_first_derivative * norm_first_derivative);
...
void draw_vector(Vector_3 v, Vector_3 v0, float red, float green, float blue) {
    // Définir la couleur du vecteur
    glColor3f(red, green, blue);

    // Dessiner le vecteur
    glBegin(GL_LINES);
    glVertex3f(v0.x(), v0.y(), v0.z()); // Point de départ du vecteur (origine)
...
void calculate_frenet(Segment s, double sk, double delta_k) {
    ...
    Vector_3 T = 
         1.0 / delta_k * (  s.B + 2.0 * ((sk - 1) / delta_k) * s.C +
                            3.0 *(((sk - 1) / delta_k), 2) * s.D );


    // Calcul de la deuxième dérivée (courbure)
    Vector_3 d_2 = 1.0 / pow(delta_k, 2) * (2 * s.C + 6 * ((sk - 1) / delta_k) * s.D);
    Vector_3 d_2_normalized = d_2 /calculate_norme(d_2);

    Vector_3 B = CGAL::cross_product(T, d_2_normalized);

    Vector_3 N = cross_product(B, T);

    Vector_3 normale = N / calculate_norme(N);
    ...
    Vector_3 tangente = T / calculate_norme(T);

    //normal

    //  binormal (produit vectoriel entre tangente et normal)
    Vector_3 produit_cross = cross_product(tangente, normale);
    Vector_3 binormale = produit_cross / calculate_norme(produit_cross);

    afficherRepereFrenet(tangente, normale, binormale, s.P0.point, 0.1);

// glPushMatrix();
//     glTranslatef(s.P0.point.x(), s.P0.point.y(), s.P0.point.z());
//     draw_vector(T, s.P0.point,  1.0f, 0.0f, 0.0f); // Rouge pour T
//     draw_vector(N, s.P0.point,0.0f, 0.0f, 1.0f); // Bleu pour N
//     draw_vector(B, s.P0.point, 0.0f, 1.0f, 0.0f); // Vert pour B
...
void afficherRepereFrenet(const Vector_3& tangente, const Vector_3& normal, const Vector_3& binormal, const Point_3& position, double length) {
    // La tangente en jaune
    glColor3f(1.0, 1.0, 0);
    glBegin(GL_LINES);
    glVertex3f(position.x(), position.y(), position.z());
    glVertex3f(position.x() + length * tangente.x(), position.y() + length * tangente.y(), position.z() + length * tangente.z());
    glEnd();
    ...
    glBegin(GL_LINES);
    glVertex3f(position.x(), position.y(), position.z());
    glVertex3f(position.x() + length * normal.x(), position.y() + length * normal.y(), position.z() + length * normal.z());
    glEnd();

    // La binormale en bleu
    glColor3f(1.0, 0.0, 1.0);
    glBegin(GL_LINES);
    glVertex3f(position.x(), position.y(), position.z());
    glVertex3f(position.x() + length * binormal.x(), position.y() + length * binormal.y(), position.z() + length * binormal.z());
    glEnd();
...
void similitude(Spline s1, Spline s2) {
    ...
    double courbure_s1 = calculate_courbure(s1_last_segment, 1.0);
    double courbure_s2 = calculate_courbure(s2_first_segment, 0.0); 

    cout << "\tcoubure s1 = " << courbure_s1 << endl;
    
    cout << "\tcoubure s2 = " << courbure_s2 << endl;
}
...
void similitude_iterative(Spline s1, Spline s2) {
    ...
    auto angle = approximate_angle(To, Ti);
    cout << "Angle approximatif entre : "<<Ti<< " et " << To << " = " << round(angle) << " degrés" << endl;
    ...
    int i = s1_segment_size-1, j = 0, seuil = 35;
    ...
    do {
        ...
        // cout << "here s1_last_segment PO" << s1_last_segment.P0.point << " and P1 " << s1_last_segment.P1.point << endl;
        // cout << "here s2_first_segment PO" << s2_first_segment.P0.point << " and P1 " << s2_first_segment.P1.point << endl;


        //TODO: verifier les TO et TI appropriés 
        To = s1_last_segment.P1.To;
        Ti = s2_first_segment.P0.Ti;
        angle = round(approximate_angle(To, Ti));
        // cout << "Angle approximatif entre : "<<Ti<< " et " << To << " = " << angle << " degrés" << endl;
    ...
    cout << "score de similitude total : " << score_total << "/" << size2 << endl;
    cout << "seuil final : " << seuil << endl;
...
int evaluate_curvature(Segment s1, Segment s2, double seuil) {
    ...
    while (s <= 1.0) {
        ...
        double courbure_s1 = calculate_courbure(s1, 1.0 - s);
        double courbure_s2 = calculate_courbure(s2, s);
        ...
        auto diff = round(courbure_s1 - courbure_s2);
        // cout << "diff : " << diff << endl;
...
vector<Spline> split_splines(Spline S, int pointID)
{
    Spline spline1, spline2;
    vector<Spline> splitSplines;
    if (pointID < 0 || pointID >= S.points.size()) {
        cout << "in split_splines pointID < 0 || pointID >= P.size()" << endl;
        return splitSplines;
    }
    for (int i = 0; i <= pointID; ++i) {
        // Ajouter les éléments au premier vecteur
        spline1.points.push_back(s.points[i]);
    }

    
    for (int i = pointID+1; i < s.points.size(); ++i) {
        // Ajouter les éléments au deuxième vecteur
        spline2.points.push_back(s.points[i]);
    }

    spline1.segments = update_segments(spline1);
    spline2.segments = update_segments(spline2);
...
Spline TCB_Spline(vector<CrestPoint> &P, double tension, double continuité, double bias) {
    Spline courbe;
    cout << "~~~~~~~~~~~~~~~~~~~~" << endl;
    cout << "SPLINE OF " << P.size() << " POINTS: " << endl;
...
*/
    ...
    cout << "NOW HAS " << sizec << "POINTS." << endl;
...
vector<Segment> update_segments(Spline courbe)
{
    vector<Segment> segments;
    Segment segment;
    vector<CrestPoint> P = courbe.points;
...
//ordonne les points de crete selon leur ordre d'apparition dans les composantes
vector<CrestPoint> traceCrestLine(const vector<CrestPoint>& crestPoints, const vector<CrestEdge>& crestEdges, int startingPointID, int componentID) {
...
void initTCBSplineTest()
{
    ...
    if (!allPoints.empty()) {
        ...
        for(int i=1; i<N+1; i++) {
                ...
                vector<pair<int, int>> endpoints = findEndpoints(allPoints, crestEdges);
                ...
                cout << "firstEndpointComponentID "  <<  endpointComponentID << endl;
                 ...
                 vector<CrestPoint> orderedPoints = allPoints;
                s = TCB_Spline(orderedPoints, tension,  continuity, bias);
                // cout << "s segment size is " << s.segments.size() << endl;


                auto numPoints = s.points.size();
                // cout << "orderedPoints has " << numPoints <<" points and s has " << s.points.size() << " points" << endl;

                vector<Spline> splines = split_splines(s, numPoints / 2);
                s1 = splines[0];
                s2 = splines[1];
                auto size1 = s1.points.size();
                auto size2 = s2.points.size();
...
void initTCBSpline()
{
    ...
    if (!allPoints.empty()) {
        ...
        for(int i=1; i<N+1; i++) {
                ...
                vector<CrestPoint> orderedPoints = traceCrestLine(allPoints, crestEdges, endpointID, endpointComponentID);
                //  vector<CrestPoint> orderedPoints = allPoints;
                s = TCB_Spline(orderedPoints, tension,  continuity, bias);
                cout << "s segment size is " << s.segments.size() << endl;
                ...
                // auto numPoints = allPoints.size();
                // cout << "allPoints has " << numPoints <<" points and s has " << s.points.size() << " points" << endl;


                auto numPoints = orderedPoints.size();
                cout << "orderedPoints has " << numPoints <<" points and s has " << s.points.size() << " points" << endl;
                // drawTCBSpline(s, tension, continuity, bias, blue);
                
                // auto numPoints = s.points.size();
                // cout << "s has " << s.points.size() << " points" << endl;


                // vector<Spline> splines = split_splines(s, numPoints / 2);
                vector<Spline> splines = split_splines(s, 700);
                ...
                cout << "s1 has " << size1 <<" points and s2 has " << size2 << " points out of " << numPoints << endl;
...
// drawPoints(s2.points, red);
    ...
    // calculate_frenet(s1.segments.at(2), 0.5, 1.0);

    // for (const auto& segment : s.segments) {
    //     calculate_frenet(segment, 0.5, 1.0);
    // }

    // for (const auto& segment : s2.segments) {
    //     calculate_frenet(segment, 0.5, 1.0);
    // }
}
